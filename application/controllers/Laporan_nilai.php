<?php
	...
	class Laporan_nilai extends CI_Controller
	{
		...
		function index()
		{
			...
			$siswa 			= "SELECT ts.nisn, ts.nama
							  FROM tbl_riwayat_kelas AS trk, tbl_siswa AS ts 
							  WHERE trk.nisn = ts.nisn AND trk.kd_kelas = '".$walikelas['kd_kelas']."' 
		...
		function nilai_semester(){
	       ...
	       $sqlSiswa = "SELECT ts.nama as nama_siswa, ts.nisn, tj.nama_jurusan, tk.nama_kelas, tk.kd_tingkatan
	                    FROM tbl_riwayat_kelas as trk, tbl_siswa as ts, tbl_kelas as tk, tbl_jurusan as tj
	                    WHERE ts.nisn=trk.nisn and tk.kd_kelas = ts.kd_kelas and tk.kd_jurusan = tj.kd_jurusan 
	                    and trk.nisn='$nisn' and trk.id_tahun_akademik=".get_tahun_akademik('id_tahun_akademik');
	       $siswa = $this->db->query($sqlSiswa)->row_array();
	        ...
	        $pdf->Cell(88,5,': '.$siswa['nisn'],0,0,'L');
	        ...
	        foreach ($mapel as $m){
	            ...
	            $nilai = check_nilai($siswa['nisn'], $m->id_jadwal);
