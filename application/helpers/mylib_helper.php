<?php
	...
	function get_tahun_akademik($field)
	{
		...
		try {
			...
			$query = $ci->db->get('tbl_tahun_akademik');

			if ($query === FALSE) {
				log_message('error', 'Database error in get_tahun_akademik: ' . $ci->db->error()['message']);
				return null;
			}

			$tahun = $query->row_array();

			if (empty($tahun)) {
				log_message('error', 'No active academic year found in get_tahun_akademik');
				return null;
			}

			if (!isset($tahun[$field])) {
				log_message('error', 'Field ' . $field . ' not found in academic year data');
				return null;
			}
			...
			log_message('error', 'Exception in get_tahun_akademik: ' . $e->getMessage());
			return null;
		}
	}
