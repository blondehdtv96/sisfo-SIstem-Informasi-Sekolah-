  ...
  <aside class="main-sidebar">
    ...
    <section class="sidebar">
      ...
      <ul class="sidebar-menu" data-widget="tree">
        ...
        // Check if user level is available and valid
        if (!empty($id_level_user) && is_numeric($id_level_user)) {
            // Use parameterized query to prevent SQL injection
            $sql_menu = "SELECT * FROM `tabel_menu` WHERE id IN(SELECT id_menu FROM tbl_user_rule WHERE id_level_user = ?) AND is_main_menu = 0";
            $main_menu  = $this->db->query($sql_menu, array($id_level_user))->result();
        } else {
            // If no user level, show empty menu or redirect to login
            log_message('error', 'Template: id_level_user is empty or invalid: ' . print_r($id_level_user, true));
            $main_menu = array(); // Empty array to prevent errors
        }
